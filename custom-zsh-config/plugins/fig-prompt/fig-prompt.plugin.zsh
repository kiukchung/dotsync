...
function get_fig_client_root() {
  local root_dir="${$(/bin/pwd -P)%/google3*}"
  echo "$root_dir"
}
...
# Returns 0 if the status cache file is old, i.e.
# if it's older than any state indicator files.
function fig_status_cache_is_old() {
  local cached_state_hash_file="$( get_fig_client_cache_dir )/$FIG_STATE_HASH_FILE"
  local fig_client_root=$( get_fig_client_root )
  for file in ${FIG_STATE_INDICATOR_FILES}; do
    file="$fig_client_root/${file}"
    # Cache should be considered old if its mtime is not newer.
    if [[ ! $file -ot $cached_state_hash_file ]]; then
      return 0
    fi
  done
  return 1
...
function should_update_fig_status_cache() {
  # Hash computation can be slow for a large fig repo with many tracked files.
  # We check the mtime and return early if cache is latest.
  # This can avoid computing the hash after users type non-fig commands.
  if ! fig_status_cache_is_old; then
    return 1  # Do not update cache
  fi
  ...
  # Do not update cache. But touch the file to indicate that it's latest.
  touch $cached_state_hash_file
  return 1
}
...
function is_fig_client() {
  if [[ "$(/bin/pwd -P)" =~ /google/src/cloud/$USER/* ]] && \
     [[ -d ${$(/bin/pwd -P)%%/google3*}/.hg ]]; then
    return 0
  fi
  return 1
}
...
#   FIG_PROMPT_BRANCH: Replaced with $branch
...
function get_fig_prompt() {
  ...
  if [ $? != 0 ] || [ $COLORS -le 8 ] || [ "${FIG_PROMPT_NO_HYPERLINKS:-unset}" != "unset" ]; then
    ...
    # Use %{...%} to tell zsh that the escape sequence has no visible length.
  ...
  tpl="${tpl//FIG_PROMPT_CLEAN/$clean}"
  ...
  tpl="${tpl//FIG_PROMPT_BRANCH/$branch}"
  tpl="${tpl//FIG_PROMPT_AHEAD/$ahead}"
  echo "$tpl"
}
